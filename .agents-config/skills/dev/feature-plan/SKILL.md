...
1. Start from the GitHub feature record and optional disposable state. Treat conversation history as non-authoritative.
...
7. Update only disposable state or render a GitHub checkpoint preview; do not publish, implement, or mark a stage complete from agent prose.
8. Use a durable handoff before a gate, provider/session switch, compaction, ownership change, or stop.
