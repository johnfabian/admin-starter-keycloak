...
4. Create the smallest decision concept using [templates/adr.md](templates/adr.md) at `wiki/decisions/ADR-####-<kebab-case-slug>.md`; the filename and `adr_id` must use the same immutable identifier.
5. Preserve earlier ADRs. Supersede with explicit bidirectional links instead of rewriting or deleting history.
6. Update the decisions index and root log, run the wiki audit, and prepare the repository path/commit cross-link for GitHub.
